...
<xsl:template match="Entry"><xsl:if test="(position() mod 36) = 0">\pagebreak<xsl:call-template name="EmployeeRow"/></xsl:if>&amp;&amp;<xsl:value-of select="AccountNumber"/>&amp;<xsl:value-of select="AccountName"/>&amp;<xsl:if test="quantity != ''"><xsl:value-of select="format-number(quantity,&quot;#'##0.00&quot;,'apodot')"/></xsl:if>&amp;<xsl:if test="rate != ''"><xsl:value-of select="format-number(rate,&quot;#'##0.0000&quot;,'apodot')"/></xsl:if>&amp;<xsl:if test="amount != ''"><xsl:value-of select="format-number(amount,&quot;#'##0.000&quot;,'apodot')"/></xsl:if>\\
</xsl:template>
