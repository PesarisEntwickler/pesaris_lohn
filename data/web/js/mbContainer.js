...
(function($){
  jQuery.fn.buildContainers = function (aafwoptions){
    return this.each (function (){
      ...
      $.extend(this.options, aafwoptions);
...
//      container.attr("options",this.options); //TODO:macht Probleme mit IE8 und musste deaktiviert werden
...
//      container.containerSetIcon(container.attr("icon"), this.options.elementsPath);
      container.containerSetIcon(container.attr("icon"), this.options.iconPath);
      ...
      if (container.hasClass("draggable")){
        ...
        container.draggable({
          ...
          start:function(){},
          stop:function(){
	    var opt=$(this).get(0).options; //TODO:neu
//            var opt=$(this).attr("options"); //TODO:alt
...
//      var opt= container.attr("options"); //TODO:alt
      var opt= this.options; //TODO:neu
      if (opt.onLoad) {
  ...
  jQuery.fn.containerResize = function (){
	...
	var el=$(this).get(0); //TODO:neu
	if(el.options.onResize) el.options.onResize($(el)); //TODO:neu
//        var opt=container.attr("options"); //TODO:alt
//        if(opt.onResize) opt.onResize(container); //TODO:alt
        if (container.attr("rememberMe")){
  ...
  jQuery.fn.containerSetButtons = function (buttons,opt){
    ...
    if (buttons !=""){
      ...
      for (var i in btn){
        ...
        if (btn[i]=="f"){
          container.find(".buttonBar:first").append("<img src='"+path+container.attr('skin')+"/max.png' class='collapsedContainer'/>");
          container.find(".collapsedContainer:first").bind("click",function(){aafw_fullscreen($(container));});
          container.find(".n:first").bind("dblclick",function(){aafw_fullscreen($(container));});
        }
        if (btn[i]=="m"){
        ...
        if (btn[i]=="i"){
          container.find(".buttonBar:first").append("<img src='"+path+container.attr('skin')+"/min.png' class='iconizeContainer'/>");
...
//      this.dockIcon= $("<img src='"+opt.elementsPath+"icons/"+(container.attr("icon")?container.attr("icon"):"restore.png")+"' class='restoreContainer' width='"+opt.dockedIconDim+"'/>").appendTo(dockPlace)
      this.dockIcon= $("<img src='"+opt.iconPath+"icons/"+(container.attr("icon")?container.attr("icon"):"restore.png")+"' class='restoreContainer' width='"+opt.dockedIconDim+"'/>").appendTo(dockPlace)
              .css("cursor","pointer")
              .hide()
...
//      if (opt.onResize) opt.onResize(container); //TODO:alt
  ...
  $.fn.adjastPos= function(margin){
    var container=$(this);
    var el=container.get(0); //TODO:neu
    var opt=el.options; //TODO:neu
//    var opt=container.attr("options"); //TODO:alt
...
//      container.setContainment(); //TODO:alt
    });
  };
